...
\def\note#1{\GenericWarning{}%
  ...
  \textsl{$\langle\!\langle${#1}$\rangle\!\rangle$}%
}
...
\title{Construction of a quasi-geodesic on a piecewise-linear sphere}
\author{Jean Chartier}
%\date{}
\begin{document}
\maketitle

\note{Define a piecewise-linear sphere. How does it differ from a polytope/a polyhedron?}

In 1949, Alexander Pogorelov proved the existence of a simple and closed quasi-geodesic on any convex polyhedron\footnote{We give in the appendix the proof that there does not always exist a straight quasi-geodesic on a convex polyhedron, i.e., a geodesic that forms two equal angles whenever it crosses a vertex.}. We describe an algorithm allowing to build such a quasi-geodesic on any piecewise-linear sphere, not necessarily convex neither embedded in $\mathhbb{R}^3$. We first prove the existence of a quasi-geodesic of length less than a bound depending on the geometric data of the sphere. This gives us an upper bound on the combinatorics of the quasi-geodesic and allows us to draw up a list of the paths to be tested. 
\begin{definition}[Quasi-geodesic]
A quasi-geodesic on a piecewise linear surface $S$ is a continuous path $\gamma : \mathbb{S}^1\longrightarrow \partial S$, where $I$ is an interval of $\mathbb{R}$, satisfying four conditions :
\begin{itemize}
\item{It is rectilinear in the interior of the faces $S$ that it crosses.}
\item{When crossing the interior of an edge of $S$, $\gamma$ subdivides $S$ locally forming two equal angles (which thus equal $\pi$).}
\item{When crossing a vertex of positive curvature, $\gamma$ subdivides $S$ locally forming two angles of value at most $\pi$.}
\item{When crossing a vertex of positive curvature, $\gamma$ subdivides $S$ locally forming two angles of value at least $\pi$.}
\end{itemize}
\end{definition}

\textbf{Notation} : Given a piecewise-linear sphere $S$ consisting of vertices $p_1,\dots,p_n$, we consider a triangulation $\mathcal{T}=\{T_1,\dots,T_l\}$ of this sphere. It is well-known that any triangulation of a $2$-sphere is \textit{shellable}, i.e. : $$\forall i=1\dots l, \bigcup_{k=1}^{k=i}T_k\simeq D^2 \text{ and }\bigcup_{k=1}^{k=l}T_k\simeq S^2.$$

For a proof, we refer for example to Ziegler~\cite{ziegler}\note{Lecture on polytopes} where it is proved that any triangulation of a $2$-sphere is polytopal (Chapter~4) and that any polytope is shellable (Chapter~8).
From now on, we can work with the combinatorics of $T$. It is immediate that a quasi-geodesic for $T$ is also a quasi-geodesic for $S$. We denote by $a_1, \dots, a_m$ the edges of this triangulation, or else if necessary, $a_{ij}$ the edge which connects the vertices $p_i$ and $p_j$\note{What does that mean?}. The union of the triangles $T_k$ having $p_i$ as a common vertex is called the \textit{hat} of $p_i$ and will be denoted by $\mathcal{C}_i$. Let us insist on the fact that $T_i$ and $T_{i+1}$ are not necessarily adjacent. Let us assume without loss of generality that $p_0\in T_1$ et $p_n\in T_l$.

\begin{theorem}[Existence]
Let $T$ be a triangulated piecewise-linear sphere. There exists a simple and closed quasi-geodesic $\gamma : \mathbb{S}^1\longrightarrow \partial P$ whose length does not exceed $M=\sum_i L(a_i)$.
...
\note{Break down the proof into 1. Definition of sweep-out, 2. Existence of sweep-out of bounded length, 3. Disk flow for curves, 4. Lemma for the disk flow for curves, 5. Disk flow for sweep-outs, 6. Lemma for the disk flow for sweep-outs and 7. Proof of the theorem.}

\begin{proof}
We start by describing a sweep-out of $T$ by simple loops, all of which are of length at most $M$. \note{Define a sweep-out} We sweep $T_1$ (resp. $T_l$) by segments $\sigma_s^1$ (resp. $\sigma_s^l$), parallel to the side opposite to $p_0$ (resp. $p_n$). Then, for $i$ from $2$ to $l-1$ :
\begin{itemize}
...
\end{itemize}
At each segment $\sigma_s^i$ through a certain $T_i$, we continuously associate (in $s$ then in $i$) the parameterization of a loop formed by the boundary of the disc $\bigcup_{k=1}^{k=i-1}T_k$, minus its intersection with $T_i$ and connected to $\sigma_s^i$ by two portions of the edge of $T_i$ (see figure below). For $i=1$ (resp. $i=l$), we rather connect $\sigma_s^i$ to $p_0$ (resp. $p_n$) by two portions of the $T_i$'s boundary. We have thus constructed a sweep-out of $S$ by circles $\beta:S^2\longrightarrow P$ -- where $S^2$ is seen as the quotient of the cylinder $[0,1]\times S^1$ by the relation which identifies the circles $(0, S^1)$ and $(1,S^1)$ to two points -- which is of degree 1 and which satisfies:
\begin{itemize}
\item{Each fiber $\beta(s,.):S^1\longrightarrow P$ is a single broken line\note{undefined} on the surface of $S$.}
...
\item{$\forall s\in]0,1[,\beta(s,.)$ is a polygon\note{undefined} of length $l_s$ drawn on $S$.}
\end{itemize}
\begin{figure}[h!]
		\begin{center}
